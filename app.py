...
def calculate_cost():
    ...
    if not quantity_text:
        result_label.config(text="Please enter a valid quantity.", font=("Arial", 13))
        return

    quantity = float(quantity_text)
    price_per_liter = GAS_PRICES.get(fuel_type, 0)
    iva_rate = float(iva_entry.get()) / 100  # Convert IVA rate from percentage to decimal
    ...
    if receipt_type == "Simple":
        ...
        receipt += f"           Price per Liter: €{price_per_liter}\n"
        receipt += f"    Total Cost: €{total_cost}\n"
        receipt += f"              IVA Tax ({iva_rate * 100}%): €{iva_amount}\n"
    else:
        receipt = f"          Fuel Type: {fuel_type}\n"
        receipt += f"            Quantity (liters): {quantity}\n"
        receipt += f"               Price per Liter: €{price_per_liter}\n"
...
calculate_button.grid(row=row, column=0, columnspan=2, pady=(20, 0))

# Create and position the result label to display the cost
result_label = tk.Label(window, text="Cost: €0.00", font=("Arial", 13))
result_label.grid(row=row + 1, column=0, columnspan=2, pady=(10, 20))
